...
const userAlreadyRegistered = async (req, res) => {
  try {
    const { email } = req.body;

    if (!email) {
      return res.status(401).json({ message: "Email necessário para verificação" });
    }
...
const completeRegister = async (req, res) => {
  try {
    ...
    const categorySelected = await Category.findOne({ category: req.body.category });

    if (!categorySelected) {
      return res
        .status(404)
        .json({ message: "Categoria não encontrada, verifique e tente novamente" });
    }

    const subCategorySelected = categorySelected.subCategory.filter(
      (subCategory) => subCategory.category === req.body.subcategory,
    )[0];
    ...
    const data = {
      ...
      service: {
        ...
        categoryName: categorySelected.category,
        subcategory: subCategorySelected.category,
      },
    };
    ...
    const updatedUser = await User.findOneAndUpdate({ _id: req.body.id }, data, {
      new: true,
    });
    ...
    const token = jwt.sign(
      { id: updatedUser._id, accountType: updatedUser.accountType },
    ...
    console.log(error);
    return res.status(400).json({ message: "Ocorreu um erro durante o cadastro, tente novamente" });
  }
};
...
const getUser = async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    return res.status(400).json({ message: "Usuário não conectado!" });
  }
};

const updateUser = async (req, res) => {
  try {
    const actualUser = await User.findOne({ _id: req.body.id });
    ...
    const updatedUser = await User.findOneAndUpdate({ _id: req.body.id }, newData, {
      new: true,
    });
    ...
    return res.status(400).json({
      message: "Ocorreu um erro durante a atualização de dados, tente novamente",
    });
  }
};
...
const changePassword = async (req, res) => {
  ...
  try {
    ...
    const isPasswordValid = await bcrypt.compare(actualPassword, user.hashedPassword);

    if (!isPasswordValid) {
      return res.status(401).json({ message: "Senha invalida! Verifique e tente novamente" });
    }

    user.hashedPassword = await bcrypt.hash(newPassword, 12);
    user.save();

    return res.status(200).json({ user, message: "Senha alterada com sucesso" });
  } catch (error) {
    console.log(error);
    return res
      .status(400)
      .json({ message: "Ocorreu um erro durante a alteração da senha, tente novamente", error });
  }
};

const buyContact = async (req, res) => {
  const { contactPrice, id, orderId } = req.body;

  try {
    const professional = await User.findOne({ _id: id });

    if (professional.coins == 0 || professional.coins < contactPrice) {
      return res.status(405).json({ message: "Moedas insuficientes" });
    }

    const updatedProfessional = await User.findOneAndUpdate(
      { _id: id },
      {
        $inc: {
          coins: -contactPrice,
        ...
        $push: {
          ordersBuyed: { orderId, createdAt: new Date() },
        },
      },
      { new: true },
    );
    ...
    console.log("Error on buyContact", error);
    return res
      .status(400)
      .json({ message: "Ocorreu um erro durante a compra do contato, tente novamente", error });
  }
};

const getBuyedOrders = async (req, res) => {
  const { userId, dateFilter } = req.body;

  try {
    const professional = await User.findOne({ _id: userId });

    const filteredOrders = professional.ordersBuyed.filter((order) => {
      if (compareAsc(new Date(order.createdAt), new Date(dateFilter)) > 0) {
        return order;
      }
    });
    ...
    for (let i = 0; i < filteredOrders.length; i++) {
      for (let j = 0; j < orders.length; j++) {
        if (filteredOrders[i].orderId.equals(orders[j]._id)) {
          const res = await axios.get(
            `https://viacep.com.br/ws/${orders[j].requesterLocation.cep.replace("-", "")}/json/`,
          );

          const city = res.data.uf;
          const state = res.data.localidade;

          result.push({
            createdAt: filteredOrders[i].createdAt,
            requesterName: orders[j].requesterName,
            category: orders[j].category,
            city,
            state,
            orderId: orders[j]._id,
            id: filteredOrders[i]._id,
            requestAnswers: orders[j].requestAnswers,
            isConcluded: filteredOrders[i].isConcluded,
          });
        }
      }
    }
    ...
    console.log("Error on getBuyedOrders", error);
    return res
      .status(400)
      .json({ message: "Ocorreu um erro durante a buscas dos pedidos comprados", error });
  }
};

const deleteBuyedOrder = async (req, res) => {
  try {
    ...
    console.log("Error on deleteBuyedOrder", error);
    return res
      .status(400)
      .json({ message: "Ocorreu um erro durante a exclusão do contato comprado", error });
  }
};

const concludeOrder = async (req, res) => {
  try {
    const { id, orderId } = req.body;

    const user = await User.findOneAndUpdate(
      { _id: id, "ordersBuyed.orderId": orderId },
      {
        $set: {
          "ordersBuyed.$.isConcluded": true,
        },
      },
      { new: true },
    );
    ...
    // http://localhost:5173/avaliacao-do-profissional?id=6531aebba7642fa35ddbd4f9&order=6536afad1caa004c548f1e22 -- url que servirá de link no email

    const sendedReview = await transport.sendMail({
      from: '"Profise" <atendimento@profise.com>', // sender address
      to: `${order.requesterEmail}`, // list of receivers
      subject: "Avaliação do profissional - Profise", // Subject line
      html: `
...
table, td { color: #000000; } #u_body a { color: #0000ee; text-decoration: underline; } @media (max-width: 480px) { #u_content_heading_1 .v-font-size { font-size: 22px !important; } #u_content_text_4 .v-container-padding-padding { padding: 40px 10px 10px !important; } #u_content_text_4 .v-text-align { text-align: center !important; } #u_content_button_1 .v-container-padding-padding { padding: 10px 10px 40px !important; } #u_content_button_1 .v-size-width { width: 65% !important; } #u_content_image_3 .v-container-padding-padding { padding: 10px 10px 40px !important; } #u_content_text_6 .v-container-padding-padding { padding: 10px 10px 20px !important; } #u_content_image_4 .v-container-padding-padding { padding: 20px 10px 40px !important; } }
    </style>
...
<table id="u_content_heading_1" style="font-family:'Raleway',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <h1 class="v-text-align v-font-size" style="margin: 0px; line-height: 140%; text-align: center; word-wrap: break-word; font-size: 35px; font-weight: 400;"><strong>Avalie sua Experiência com o Profissional</strong></h1>
...
<div class="v-text-align" align="center">
  <!--[if mso]><v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="https://unlayer.com" style="height:37px; v-text-anchor:middle; width:162px;" arcsize="11%"  stroke="f" fillcolor="#407bff"><w:anchorlock/><center style="color:#FFFFFF;"><![endif]-->
    <a href="http://localhost:5173/avaliacao-do-profissional?id=${id}&order=${orderId}" target="_blank" class="v-button v-size-width v-font-size" style="box-sizing: border-box;display: inline-block;text-decoration: none;-webkit-text-size-adjust: none;text-align: center;color: #FFFFFF; background-color: #407bff; border-radius: 4px;-webkit-border-radius: 4px; -moz-border-radius: 4px; width:58%; max-width:100%; overflow-wrap: break-word; word-break: break-word; word-wrap:break-word; mso-border-alt: none;font-size: 14px;">
      <span style="display:block;padding:10px 20px;line-height:120%;"><span style="line-height: 16.8px;">Avaliar Profissional</span></span>
    </a>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <td class="v-text-align" style="padding-right: 0px;padding-left: 0px;" align="center">
      ...
      <img align="center" border="0" src="cid:image-2" alt="image" title="image" style="outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;clear: both;display: inline-block !important;border: none;height: auto;float: none;width: 88%;max-width: 246.4px;" width="246.4"/>
...
<!--[if (mso)|(IE)]><td align="center" width="600" style="background-color: #407bff;width: 600px;padding: 0px;border-top: 0px solid transparent;border-left: 0px solid transparent;border-right: 0px solid transparent;border-bottom: 0px solid transparent;border-radius: 0px;-webkit-border-radius: 0px; -moz-border-radius: 0px;" valign="top"><![endif]-->
<div class="u-col u-col-100" style="max-width: 320px;min-width: 600px;display: table-cell;vertical-align: top;">
  <div style="background-color: #407bff;height: 100%;width: 100% !important;border-radius: 0px;-webkit-border-radius: 0px; -moz-border-radius: 0px;">
  <!--[if (!mso)&(!IE)]><!--><div style="box-sizing: border-box; height: 100%; padding: 0px;border-top: 0px solid transparent;border-left: 0px solid transparent;border-right: 0px solid transparent;border-bottom: 0px solid transparent;border-radius: 0px;-webkit-border-radius: 0px; -moz-border-radius: 0px;"><!--<![endif]-->
...
<table id="u_content_text_6" style="font-family:'Raleway',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <div class="v-text-align v-font-size" style="font-size: 14px; color: #ffffff; line-height: 170%; text-align: center; word-wrap: break-word;">
    <p style="line-height: 170%;">POLÍTICAS DE PRIVACIDADE   |   SITE</p>
<p style="line-height: 170%;"> </p>
<p style="line-height: 170%;">Profiser: Conectando Você aos Melhores Profissionais!</p>
  </div>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <td class="v-text-align" style="padding-right: 0px;padding-left: 0px;" align="center">
      ...
      <img align="center" border="0" src="cid:image-1" alt="image" title="image" style="outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;clear: both;display: inline-block !important;border: none;height: auto;float: none;width: 100%;max-width: 150px;" width="150"/>
...
`, // html body
    ...
    console.log("Error on concludeOrder", error);
    return res.status(400).json({
      message: "Ocorreu um erro durante o processo de marcar como concluído o contato",
      error,
    });
  }
};

const reviewedProfessional = async (req, res) => {
  try {
    const { id } = req.body;
    ...
    const userData = {
      ...
      category: user.service.categoryName,
      subCategory: user.service.subcategory,
    };

    return res.status(200).json(userData);
  } catch (error) {
    console.log("Error on reviewedProfessional", error);
    return res.status(400).json({
      message: "Ocorreu um erro durante a busca do profissional, tente novamente mais tarde",
      error,
    });
  }
};
...
const sendReview = async (req, res) => {
  try {
    ...
    if (!userUpdated) {
      return res.status(404).json({ message: "Profissional não encontrado", sended: false });
    }

    return res.status(200).json({ sended: true });
  } catch (error) {
    console.log("Error on sendReview", error);
    return res.status(400).json({
      message:
        "Ocorreu um erro durante o envio da avaliação para o profissional, tente novamente mais tarde",
      error,
      sended: false,
    });
  }
...
const requestPasswordChange = async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(404)
        .json({ message: "Email é necessário para fazer a solicitação", sended: false });
    }

    const user = await User.findOne({ email });

    if (!user) {
      return res.status(404).json({ message: "Email inserido não está cadastrado", sended: false });
    }
    ...
    await transport.sendMail(
      {
        ...
        subject: "Recuperação de Senha - Profise", // Subject line
        html: `
<!DOCTYPE HTML PUBLIC "-//W3C//DTD XHTML 1.0 Transitional //EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
<head>
...
table, td { color: #000000; } #u_body a { color: #0000ee; text-decoration: underline; } @media (max-width: 480px) { #u_content_heading_1 .v-font-size { font-size: 22px !important; } #u_content_text_2 .v-container-padding-padding { padding: 20px 10px 10px !important; } #u_content_button_1 .v-container-padding-padding { padding: 10px 10px 40px !important; } #u_content_button_1 .v-size-width { width: 65% !important; } #u_content_text_6 .v-container-padding-padding { padding: 10px 10px 20px !important; } #u_content_image_4 .v-container-padding-padding { padding: 20px 10px 40px !important; } }
    </style>
...
<table id="u_content_heading_1" style="font-family:'Raleway',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <h1 class="v-font-size" style="margin: 0px; line-height: 140%; text-align: center; word-wrap: break-word; font-size: 35px; font-weight: 400;"><strong>Recuperação de Acesso à Sua Conta</strong></h1>
...
<table id="u_content_text_2" style="font-family:'Raleway',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <div class="v-font-size" style="font-size: 14px; line-height: 140%; text-align: center; word-wrap: break-word;">
    <p style="line-height: 140%;"><span data-metadata="&lt;!--(figmeta)eyJmaWxlS2V5IjoiaVppZVJ3elpVbXhzTkswWkZOMThraiIsInBhc3RlSUQiOjE2NjAxMjUwNzksImRhdGFUeXBlIjoic2NlbmUifQo=(/figmeta)--&gt;" style="line-height: 19.6px;"></span><span data-buffer="&lt;!--(figma)(/figma)--&gt;" style="line-height: 19.6px;"></span>Você solicitou a recuperação de senha da sua conta no Profise. Siga as instruções abaixo para criar uma nova senha e recuperar o acesso à sua conta.</p>
  </div>
...
<div align="center">
  <!--[if mso]><v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="https://unlayer.com" style="height:37px; v-text-anchor:middle; width:174px;" arcsize="11%"  stroke="f" fillcolor="#407bff"><w:anchorlock/><center style="color:#FFFFFF;"><![endif]-->
    <a href="http://localhost:5173/recuperacao-de-senha?id=${user._id}" target="_blank" class="v-button v-size-width v-font-size" style="box-sizing: border-box;display: inline-block;text-decoration: none;-webkit-text-size-adjust: none;text-align: center;color: #FFFFFF; background-color: #407bff; border-radius: 4px;-webkit-border-radius: 4px; -moz-border-radius: 4px; width:30%; max-width:100%; overflow-wrap: break-word; word-break: break-word; word-wrap:break-word; mso-border-alt: none;font-size: 14px;">
...
<table id="u_content_text_6" style="font-family:'Raleway',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <div class="v-font-size" style="font-size: 14px; color: #ffffff; line-height: 170%; text-align: center; word-wrap: break-word;">
    <p style="line-height: 170%;">POLÍTICAS DE PRIVACIDADE   |   SITE</p>
<p style="line-height: 170%;"> </p>
<p style="line-height: 170%;">Profise: Conectando Você aos Melhores Profissionais!</p>
  </div>
...
`, // html body
      ...
      (error) => {
        if (error) {
          console.log("Error on Email of requestPasswordChange", error);
          return res.status(400).json({
            message: "Ocorreu um erro durante o envio do email, tente novamente mais tarde",
            error,
            sended: false,
          });
        }
    ...
    console.log("Error on requestPasswordChange", error);
    return res.status(400).json({
      message:
        "Ocorreu um erro durante a solicitação de recuperação de senha, tente novamente mais tarde",
      error,
      sended: false,
    });
  }
...
const passwordRecovery = async (req, res) => {
  try {
    ...
    if (password !== passwordConfirm) {
      return res
        .status(401)
        .json({ message: "As senhas não coincidem, verifique e tente novamente." });
    }
    ...
    return res.status(200).json({ message: "Senha restaurada com sucesso" });
  } catch (error) {
    console.log("Error on passwordRecovery", error);
    return res.status(400).json({
      message: "Ocorreu um erro durante a recuperação da senha, tente novamente mais tarde",
      error,
    });
  }
};
