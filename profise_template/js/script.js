(function($) {
    ...
    $(".job-search-form-two input").keyup(function() {
        
        var valor = $(this).val();
        
        if(valor.length > 2) {
            
            $.get( "https://www.profise.com.br/api/categories/?format=json&name=" + valor +"", function( data ) {
                
                let array = $.map(data, function(el) { return el; })
                ...
                for (var i = 0; i < tamanho; i++) {
                    ...
                    string += "<li onclick=\"atualizaCampo('" + array[i]["name"] + "')\">"+array[i]["name"]+"</li>";
                    
                }
                
                $(".sugestoes ul").html(string);
